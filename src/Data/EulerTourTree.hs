...
instance (Ord node) => Semigroup (EulerTourMonoid node) where
  EulerTourMonoid a b c d e <> EulerTourMonoid a' b' c' d' e' = result
    where
...
fromTree tree = do
  ...
  where
    ...
    fromTree' (T.Node node forest) = n FT.<| mconcat ((FT.|> n) . fromTree' <$> forest)
      where
...
toTree (EulerTourTree fingerTree) = MS.evalStateT parser fingerTree
  where
    parser = do
...
cutEdge (EulerTourTree tree) e@(a, b) = do
  ...
  where
    p1 before after = edgeMember e before
...
splice (EulerTourTree inviteeTree) node (EulerTourTree hostTree) = do
  ...
  where
    p before after = vertexMember node before && not (vertexMember node after)
...
reroot node (EulerTourTree tree) = do
  ...
  where
    p1 before after = vertexMember node before
